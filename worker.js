...
// 账号池热加载: 注册服务写入 credentials/*.json 后, worker 下次请求自动发现新账号
import { readFileSync, readdirSync, existsSync, statSync, writeFileSync } from 'node:fs';
import { resolve, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';

// 凭据目录: Docker 内 /app/credentials（挂载宿主机 credentials/），本地直跑时与
// worker.js 同级的 credentials/。两种部署下该目录都与 worker.js 同目录，按模块
// 位置解析即可 —— 否则本地 node server.js 直跑时读不到 banned_tokens.json，
// 重启后 banned 账号会重新参与调度（且凭据目录热加载失效）。
const CRED_DIR = resolve(dirname(fileURLToPath(import.meta.url)), "credentials");
let tokenPoolCache = null;             // string[] 当前 token 列表
let tokenPoolSig = "";                 // 文件签名 (mtime 拼接), 变化才重读

// ---------------------------------------------------------------------------
// banned 账号持久化（v1.9.0）：被封禁(403 status=banned, Terminal)或手动移除的
// token 写入 credentials/banned_tokens.json，随挂载卷持久化；重启后依然跳过。
// 文件格式: { "tokens": ["..."], "updatedAt": "ISO" }
// ---------------------------------------------------------------------------
const BANNED_FILE = resolve(CRED_DIR, "banned_tokens.json");
let bannedTokens = new Set();          // token -> true（持久化 banned 名单）
let bannedFileSig = "";                // banned 文件 mtime 签名

function loadBannedTokens() {
  try {
    if (existsSync(BANNED_FILE)) {
      const st = statSync(BANNED_FILE);
      const sig = String(st.mtimeMs);
      if (sig !== bannedFileSig) {
        const obj = JSON.parse(readFileSync(BANNED_FILE, "utf-8"));
        bannedTokens = new Set((Array.isArray(obj?.tokens) ? obj.tokens : [])
          .map((t) => String(t).trim()).filter((t) => t.length > 8));
        bannedFileSig = sig;
      }
    } else {
...
// 把 token 写入持久化 banned 名单（幂等）。失败不抛错，仅记录。
function persistBannedToken(token) {
  if (!token || token.length <= 8) return;
  try {
    loadBannedTokens();
    bannedTokens.add(token);
    const obj = { tokens: [...bannedTokens], updatedAt: new Date().toISOString() };
    writeFileSync(BANNED_FILE, JSON.stringify(obj, null, 2), "utf-8");
    bannedFileSig = String(statSync(BANNED_FILE).mtimeMs);
  } catch (e) {
    console.error("[worker] persist banned token failed:", e.message);
  }
}
...
function isBannedToken(token) {
  ...
  loadBannedTokens();
  if (bannedTokens.has(token)) return true;
  const h = acctHealth.get(token);
  return !!(h && h.state === "banned");
}

// 把 token 从持久化 banned 名单移除（恢复参与调度），并清内存观测状态
function unbanTokenInternal(token) {
  if (!token || token.length <= 8) return;
  try {
    loadBannedTokens();
    if (!bannedTokens.has(token)) return;
    bannedTokens.delete(token);
    const obj = { tokens: [...bannedTokens], updatedAt: new Date().toISOString() };
    writeFileSync(BANNED_FILE, JSON.stringify(obj, null, 2), "utf-8");
    bannedFileSig = String(statSync(BANNED_FILE).mtimeMs);
  } catch (e) {
    console.error("[worker] unban token failed:", e.message);
    return;
  }
  // 清内存观测状态，让账号重新参与调度（下次请求重新探测）
  acctHealth.delete(token);
  cooldowns.delete(token);
  // 清除该账号全部 client 的 session 缓存
  invalidateSessionCache(token);
}

function loadTokenList(env) {
  let tokens = [];
  let sig = "";
  try {
    if (existsSync(CRED_DIR)) {
      const files = readdirSync(CRED_DIR).filter((f) => f.endsWith(".json")).sort();
      const parts = [];
      ...
      sig = parts.join(";");
      if (sig === tokenPoolSig && tokenPoolCache) return tokenPoolCache;
      for (const f of files) {
        try {
  ...
  if (tokens.length === 0) {
    // 无凭据文件时回退环境变量
    tokens = (env.FREEBUFF_TOKEN || "").split(/[\n,]/).map((s) => s.trim()).filter((s) => s.length > 8);
  }
  tokenPoolCache = tokens;
  tokenPoolSig = sig;
  return tokens;
}
...
// fork 后请改为你自己的仓库（asset 由 .github/workflows/build-models-release.yml 自动发布）
const DYNAMIC_MODELS_RELEASE_SOURCES = [
  "https://github.com/VentourF/freebuff-proxy/releases/latest/download/freebuff-models.json",
];
...
function buildDynamicModelTable(agentMappings) {
  ...
  return Object.entries(mappings.root).map(([modelId, rootAgent]) => {
    // ⚠️ 2026-08-27 实测：base2-free-* 已被官方标记 retired（403 free_mode_legacy_luna_agent），
    // 必须优先使用 base3-free-* agent（FREEBUFF_WEB_BASE3_AGENT_ID_BY_MODEL）。
    // 普通 chat 的 startRunChain 用 agent 字段，因此这里直接把它指向 base3。
    const base3 = mappings.base3[modelId] || null;
    return {
      id: modelId,
      session: modelId,
      agent: base3 || rootAgent,
...
// 白名单内硬编码（用户要求仅保留以下模型）：
//   - deepseek/deepseek-v4-pro     PREMIUM（base2-free-deepseek）
//   - deepseek/deepseek-v4-flash   STANDARD（base2-free-deepseek-flash）
//   - openai/gpt-5.6-luna          PREMIUM（base2-free-luna）
// 其余模型不再对外暴露（动态官方清单在白名单外一律过滤）。
// 本地增强：deepseek/deepseek-v4-flash 官方 main 分支已移出免费模型表，
// 但服务端仍窗口制开放（现网实测可用窗口 = 北京时间 18:00 ～ 次日 08:00，
// 即 GMT+8 18:00-08:00，2026-08-27 实测确认；旧注释的 9am ET-5pm PT 已过时），
// 硬编码补回以支持窗口期使用。
const MODELS = [
  // deepseek/deepseek-v4-pro：Premium 池（官方 limit 1/天），root agent
  // 官方映射 = base3-free-deepseek（free-agents.ts FREEBUFF_WEB_BASE3_AGENT_ID_BY_MODEL）
  // ⚠️ 2026-08-27 实测：base2-free-* 已被官方标记 retired（free_mode_legacy_luna_agent 403），
  //    必须使用 base3-free-* 且 codebuff_metadata 带 cost_mode:"free" 才能正常走免费通道。
  ...
  // openai/gpt-5.6-luna：官方 RECOMMENDED 默认模型（强全能），官方 base3 root agent
  // 映射为 base3-free-luna。
  { id: "openai/gpt-5.6-luna", session: "openai/gpt-5.6-luna", agent: "base3-free-luna", upstream: "openai/gpt-5.6-luna" },
];

// ---------------------------------------------------------------------------
// 模型列表：/v1/models 只对外列出以下模型（其余不显示，DSH 客户端也依此呈现）。
// 注意：这里只收敛"对外可见"的模型列表，不限制实际可解析/可调用的模型——
// findModelConfig/resolveModelConfig 仍会从动态官方清单解析 mimo 等其它模型。
// 动态官方清单（free-agents.ts）会额外带出 mimo/gpt-5.6-pro/luna-es 等模型，
// /v1/models 用下面的列表过滤，保证对外固定为：
...
// ---------------------------------------------------------------------------
// 官方暂停/不可用模型：
// 官方已从免费模式 WITHDRAWN / PAUSED 的模型（2026-08 官方源码确认）：
//   - minimax/minimax-m3：WITHDRAWN 2026-08-20（FREEBUFF_PAUSED_FREE_MODEL_IDS）
// （deepseek/deepseek-v4-pro 已临时放行测试——若上游仍拒绝，后续再收回）
// ---------------------------------------------------------------------------
...
// 预生成 instance-id 的桌面版签名。
// v1.9.3：不再使用 include-unused-rate-limits 额度快照头 —— 请求它会消耗账号正常次数。
// ---------------------------------------------------------------------------
...
function sessKey(clientId, token, sessionModel) {
  return (clientId ? clientId + "|" : "") + token + ":" + sessionModel;
}

// 该账号的 session 是否被"其他客户端"租用（用于 createSession 的 GET 复用前检查，
// 避免 B 客户端轮询到 A 已租用的账号时直接复用 A 的 session 导致 409 打架）
function isTokenClaimedByOther(token, clientId) {
  for (const key of sessCache.keys()) {
    if (!key.includes(":" + token + ":")) continue;
    const claimClient = key.split("|")[0];
    if (claimClient !== clientId) return true;
  }
  return false;
}
...
// v1.9.3：不再记录 rateLimitsByModel 额度快照 —— 拉取快照会消耗账号正常使用次数，
// 仅保留 banned/rate_limited 等账号状态观测（来自真实请求响应，不额外探测）。
function recordAccountObservation(token, status, dataOrText, extra = {}) {
  ...
  if (status === 404) state = "ok";
    ...
    // free_mode_legacy_luna_agent 是模型/agent 级别的错误(403+error,非账号状态),
    // 不记为 blocked 以免该账号被标记失效(账号本身健康). 仅计入 but 不影响 alive.
    const isLegacyAgent = upstreamState === null && typeof data?.error === "string" &&
      data.error.includes("free_mode_legacy_luna_agent");
    state = upstreamState === "banned"
      ? "banned"
      : upstreamState === "country_blocked" ? "country_blocked" : (isLegacyAgent ? "ok" : "blocked");
  ...
  // v1.9.0: banned 是终态，发现即写入持久化名单（重启后跳过该号）
  if (state === "banned") persistBannedToken(token);
}

function summarizeAccountHealth(pool, health) {
  const banned_count = pool.filter((acct) => isBannedToken(acct.token)).length;
  const account_details = pool.map((acct) => {
    ...
    return {
      ...
      banned: isBannedToken(acct.token), // 持久化名单或内存观测 banned
    };
  });
  const nonBanned = account_details.filter((d) => !d.banned);
  const account_states = {};
  for (const detail of nonBanned) {
    account_states[detail.state] = (account_states[detail.state] || 0) + 1;
  }
  const alive_accounts = nonBanned.filter((p) => p.alive === true).length;
  const unknown_accounts = nonBanned.filter((p) => p.alive === null).length;
  const unhealthy_accounts = nonBanned.filter((p) => p.alive === false).length;
  const status = nonBanned.length === 0
    ? "critical"
  ...
  return {
    status,
    accounts: nonBanned.length,
    banned_removed: banned_count,
    alive_accounts,
    unknown_accounts,
    account_states,
    account_details,
...
function pickToken(env, sessionModel, clientId = "") {
  const pool = parseAccounts(env);
  if (pool.length === 0) return null;

  // 记录"最近被选用"时间, 供管理页面展示正在使用的账号
  const markUsed = (acct) => { lastUsed.set(acct.token, Date.now()); return acct; };

  // v1.9.0：banned 是终态（官方语义 Terminal，不可恢复），从账号池永久剔除：
  // 不进轮询、不进回退池，chat 循环也不会选到它。判定 = 内存观测 banned
  // 或持久化 banned_tokens.json 名单（重启后依然跳过）。
  const poolWithoutBanned = pool.filter((acct) => !isBannedToken(acct.token));
  if (poolWithoutBanned.length > 0) {
    for (const acct of pool) {
      if (isBannedToken(acct.token)) sessCache.delete(sessKey(clientId, acct.token, sessionModel || ""));
    }
  }

  // v1.6.0：跳过已探测为失效的号（alive=false）；未探测/探测失败的不跳过（避免误杀）
  // v1.9.2：失效观测带 TTL 自愈 —— 403 blocked 多为出口 IP 被瞬时风控（非封号终态），
  // 观测超过 HEALTH_OBSERVATION_TTL_MS 后视为过期，重新参与调度，由下一次真实
  // 请求重新观测（200 即恢复健康）。否则一次瞬时 403 会把号永久卡死在失效列表。
  // banned 终态不受影响（已在上一步 isBannedToken 过滤，且持久化）。
  const alivePool = poolWithoutBanned.filter((acct) => {
    const h = acctHealth.get(acct.token);
    if (!h || h.alive !== false) return true;
    return Date.now() - (h.checkedAt || 0) > HEALTH_OBSERVATION_TTL_MS;
  });
  // 全失效时回退到"剔除 banned 后的全池"（banned 永远不回退），让请求继续（由 429 冷却接管）
  const usePool = alivePool.length > 0 ? alivePool : poolWithoutBanned;
  const finalPool = usePool;
  if (finalPool.length === 0) return null;

  // 优先复用已有活跃 session 缓存的号（限本客户端）：一个 session 约 1 小时有效，创建 session 才扣
  // 免费额度（如 v4-pro 每天 6 次）。纯轮询会让每个请求都切号、各建一个 session，
  // 浪费创建额度。只要当前模型的 session 缓存还活跃就钉在同一个号上，用满再换。
  // v1.9.1：sessCache key 带 clientId 前缀 —— 多主机共用反代时各钉各的账号，
  // 不会因为 A 主机的 session 缓存而把 B 主机的请求也钉到同一账号。
  if (sessionModel) {
    for (const acct of finalPool) {
      const t = acct.token;
      if (cooldowns.has(t) && cooldowns.get(t) > Date.now()) continue;
      const cached = sessCache.get(sessKey(clientId, t, sessionModel));
  ...
  const sorted = [...finalPool].map((acct) => {
    const t = acct.token;
    return { acct, remaining: (cooldowns.has(t) && cooldowns.get(t) > Date.now()) ? -Infinity : 0 };
  }).sort((a, b) => {
  ...
  for (const { acct } of sorted) {
    const t = acct.token;
    if (!cooldowns.has(t) || cooldowns.get(t) <= Date.now()) return markUsed(acct);
  }
  const oldest = [...cooldowns.entries()].sort((a, b) => a[1] - b[1])[0];
  if (oldest) cooldowns.delete(oldest[0]);
  return markUsed(finalPool[0]);
}

// 供 admin.js 调用：把 token 加入持久化 banned 名单（手动移除账号时双保险）
export function banToken(token) {
  persistBannedToken(token);
}

// 供 admin.js 调用：把 token 从 banned 名单移除，恢复参与调度
export function unbanToken(token) {
  unbanTokenInternal(token);
}

// 供 admin.js 查询：token 是否在 banned 名单（含内存观测）
export function isBannedTokenPublic(token) {
  return isBannedToken(token);
}

// 账号池状态快照: 供管理页面展示 健康/冷却/使用中
// v1.9.3：不再显示 quota（剩余额度）—— 拉取额度快照会消耗账号正常次数。
export function getAccountStatus(env) {
  const pool = parseAccounts(env);
  const now = Date.now();
  return {
    now,
    accounts: pool.map((acct) => {
      const h = acctHealth.get(acct.token);
      const cd = cooldowns.get(acct.token);
      return {
        token: acct.token,
        alive: h ? h.alive : null,
        state: h ? h.state : "unknown",
        removed: isBannedToken(acct.token), // banned 终态: 已从可用池移出（含持久化名单）
        checkedAt: h ? h.checkedAt : null,
        // 失效观测已过期：重新参与调度，等待下一次真实请求重新观测（自愈中）
        staleFail: !!(h && h.alive === false && now - (h.checkedAt || 0) > HEALTH_OBSERVATION_TTL_MS),
        uid: h && h.uid ? h.uid : null,
        cooldownUntil: cd || null,
        cooldownRemainingMs: cd ? Math.max(0, cd - now) : 0,
        lastUsedAt: lastUsed.get(acct.token) || null,
      };
    }),
  };
}
...
// v1.9.3：不再按剩余额度排序 —— 拉取额度快照会消耗账号正常使用次数。
// 账号选择退化为简单轮询（优先复用活跃 session，否则冷却跳过）。
function remainingQuota(token, sessionModel) {
  return null;
}

// v1.9.3：不再依赖额度快照判断耗尽 —— 拉取快照会消耗次数。
// 仅根据账号状态（banned/rate_limited/blocked 等）判断是否应跳过该号。
// 这些状态来自真实请求响应，不额外探测。
function isQuotaExhausted(info, sessionModel) {
  if (!info) return false;
  return ["rate_limited", "banned", "country_blocked", "token_invalid", "blocked", "model_locked", "ip_capped"].includes(info.state);
}
...
function invalidateSessionCache(token) {
  for (const key of sessCache.keys()) {
    // key 格式: `${clientId}|${token}:${model}` 或 `${token}:${model}`
    if (key.includes(":" + token + ":")) sessCache.delete(key);
  }
}
...
// v1.9.3：流式无首数据时不再做额度探测 —— 拉取/刷新额度快照会消耗账号正常次数。
// 只读本地缓存中的账号状态（banned/rate_limited 等），不判定额度耗尽。
...
async function createSession(token, sessionModel, forceCreate = false, clientId = "") {
  // 0) 正常客户端行为：广告链 + usage 触碰（30 分钟节流，失败静默）
  try { await runNormalClientBehavior(token, stableFingerprint(token)); } catch {}
  // 1) 缓存命中且未过期（剩 >60s）直接复用（仅本客户端），避免每次请求都打上游 session 接口
  if (!forceCreate) {
    const cached = sessCache.get(sessKey(clientId, token, sessionModel));
    if (isUsableSession(cached)) {
      return cached;
    }
    if (cached) sessCache.delete(sessKey(clientId, token, sessionModel));
  }
  // 1) 查上游当前 session，同模型直接复用（forceCreate 时跳过：僵尸 active session 会被 GET 反复复用，
  //    导致 chat 一直 428；强制 POST 拿全新实例）
  //    ⚠️ v1.9.1：若该账号的 session 被其他客户端租用（多主机共用反代），不复用，
  //    交给上层换号 —— 否则 B 主机可能抢到 A 主机的 session 导致 409 互相顶。
  //    v1.9.3：不再带 include-unused-rate-limits 头 —— 请求额度快照会消耗账号正常次数。
  if (!forceCreate && !isTokenClaimedByOther(token, clientId)) {
    ...
    if (cur.status === 200 && cur.data?.status === "active" && cur.data?.instanceId) {
      ...
      if (!cm || cm === sessionModel) {
        ...
        sessCache.set(sessKey(clientId, token, sessionModel), s);
        return s;
      }
  ...
  if (r.status === 409) {
    // 409 可能有两种情况：
    // 1) session_model_mismatch（模型与 session 不匹配）
    // 2) model_unavailable（窗口制模型当前不可用，如 flash 的 9am ET-5pm PT）
    // 后者 body 中 status === "model_unavailable"，提取 availableHours 给友好提示。
    if (r.data && r.data.status === "model_unavailable" && r.data.availableHours) {
      throw new ModelUnavailableError(sessionModel, `Model "${sessionModel}" is only available ${r.data.availableHours} (current time is outside this window).`);
    }
    throw new Error("session_model_mismatch: " + String(r.data?.message || r.data?.error || "上游拒绝该模型"));
  }
...
function buildUpstreamPayload(params, mc, sess, runId) {
  ...
  payload.codebuff_metadata = {
    ...
    // ⚠️ 2026-08-27 实测（A/B 剥字段定位）：
    //   base3-free-* + run_id + cost_mode:"free" 三者同时存在 = 免费通道正常（cost=0）。
    //   单独 run_id（无 cost_mode）→ 落入计费路径 → 402 Out of credits。
    //   旧 base2-free-* agent + cost_mode:"free" → 403 free_mode_legacy_luna_agent（retired）。
    // 因此必须用 base3-free-* agent，且必须带 cost_mode:"free"。
    cost_mode: "free",
  };
  return payload;
}
...
async function executeCodeReview(env, chatParams, mc, isStream, mode, clientId = "default") {
  ...
  for (let acctTry = 0; acctTry < pool.length; acctTry++) {
    ...
    try {
      const sess = await createSession(token, mc.session, false, clientId);
      ...
      if (e instanceof ModelUnavailableError) {
        return jsonResponse({ error: { message: e.message || ("Model not available upstream: " + e.modelId), type: "unsupported_model" } }, 400);
      }
...
async function executeChat(env, chatParams, mc, isStream, mode, clientId = "default") {
  if (isCodeReviewRequest(chatParams)) return executeCodeReview(env, chatParams, mc, isStream, mode, clientId);
  const debug = env.FREEBUFF_DEBUG === "true";
  const pool = parseAccounts(env);
  ...
  for (let acctTry = 0; acctTry < pool.length; acctTry++) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        ...
        try {
          ...
          if (error instanceof EmptyUpstreamStreamError && attempt === 0) {
            ...
            sessForChat = await createSession(token, mc.session, true, clientId);
            continue;
          }
          throw error;
        }
      ...
      if (e instanceof QuotaExhaustedError) {
        sessCache.delete(sessKey(clientId, token, mc.session));
...
// /v1/models 返回可用模型：硬编码 MODELS + 动态官方清单。
// 官方 WITHDRAWN / PAUSED 的模型（如 v4-pro、minimax-m3）不显示，
// 且请求时返回明确的 unsupported_model（resolveModelConfig 拦截）。
// ⚠️ 不要在这里查上游 GET /api/v1/freebuff/session（额度/状态）：
// 该接口会占用账号 session，而 Freebuff 一个号同一时间只能一个客户端在线，
// 查询会干扰/顶掉正在进行的 chat 会话（428 waiting_room_required）。
async function handleModels() {
  // 合并硬编码 + 动态清单，过滤官方已撤下/暂停的模型
  let allModels = [...MODELS];
  try {
    const dyn = await refreshDynamicModelsIfStale();
    if (dyn && dyn.models) {
      for (const m of dyn.models) {
        if (!allModels.some((x) => x.id === m.id)) allModels.push(m);
      }
    }
  } catch { /* 动态源失败时只用硬编码 */ }
  const modelList = allModels.filter((m) => !isPausedModel(m.id));
  return jsonResponse({
    object: "list",
