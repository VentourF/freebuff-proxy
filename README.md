...
1. **钉号（会话延续）**：本客户端（clientId）在该模型下已有活跃 session 的账号优先复用 —— 同一会话的多轮对话钉在同一账号，复用 session **不扣额度**；
2. **空闲优先（并发分散）**：本客户端无活跃 session（新会话）时，优先调度**空闲账号** —— 未被其他客户端租用 session、未被其他请求占用处理中、未冷却的号，轮询分散。同时打开多个会话时，各会话自动落到不同账号，不会全部堆在第一个账号上；
3. **回退轮询**：全部账号忙/冷却时按原轮询（冷却排最后）兜底。

> 💡 **并发分散无需配置**：账号"占用中"标记由 Worker 运行时维护 —— 请求选中账号即标记，
> 流式请求保持到流结束（3 分钟 TTL 兜底防泄漏），成功/失败/换号时自动释放。
> 无需给会话配置 `user` 或请求头即可分散。
>
> 💡 想让同一客户端下的多个会话进一步按标识隔离，可给每个会话配置不同的
> `user` 字段（OpenAI）/ `x-freebuff-client` 请求头 —— clientId 优先级：
> `x-freebuff-client` > `x-client-id` > 请求体 `user` > 默认 `default`。
>
> ⚠️ 注意：账号池数量最好大于并发会话数；并发会话超过账号数时，超出部分会回退复用
> （避免排队干等），账号池充足时分散效果最佳。
