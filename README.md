...
> 💡 **注册服务不跑在 Docker**：账号注册服务（`register_service.py`）在宿主机本地运行，
> 用 `start-register.ps1`（或 `python register_service/register_service.py`）启动于 8899 端口，
> worker 容器经 `host.docker.internal:8899` 访问，无需（也不建议）容器化。
