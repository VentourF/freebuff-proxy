...
// Admin routes: /accounts page + /admin/api/* (账号注册/管理)
const { handleAdmin } = await import('./admin.js');
...
const server = createServer(async (nodeReq, nodeRes) => {
  try {
    ...
    // Admin routes first (账号管理页面 + API)
    if (request.url.includes('/accounts') || request.url.includes('/admin/')) {
      await handleAdmin(request, nodeRes);
      return;
    }
